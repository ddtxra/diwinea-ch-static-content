...
$k(document).ready(function($k){ 
  ...
  $k('.nav-responsive').click(function() {
	...
	$k(document).on('click', ".header-cart", function() {
		$k("#panel").slideToggle(500);
	});
  
  $k('#narrow-by-list ol').on('click touchstart', function(){
    e.preventDefault();
    $k(this).toggleClass('hover');
    //alert();
  });
...
$k(window).resize(function(){menuResponsive(); navmenu();});   
...
function navmenu(){  
  			if ($k(window).scrollTop() > 185) {
  				$k('.nav-container').addClass("f-nav");
          $k('.header-container').addClass("f-nav-head");
  				$k(".header-cart").addClass('fixed');
          $k("a.logo").addClass('fixed');
  				$k(".header-search").addClass('fixed');
          $k("#categoryHeaderCustom #chInner").addClass('fixed');	
  			} else {
  				$k('.nav-container').removeClass("f-nav");
  				$k(".header-cart").removeClass('fixed');
          $k('.header-container').removeClass("f-nav-head");
          $k("a.logo").removeClass('fixed');
  				$k(".header-search").removeClass('fixed');
          $k("#categoryHeaderCustom #chInner").removeClass('fixed');	
  			}
}
